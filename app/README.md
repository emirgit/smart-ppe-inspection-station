...
- **Date range picker** on Analytics (last 7 days / 30 days / custom)
...
| `/analytics` | Analytics | Last 7 days / 30 days / custom date range, theme-aware chart colors |
...
- `listPpeItems`, `createPpeItem`, `updatePpeItem`, `deletePpeItem`
- `listEntryLogs(query?)`, `getEntryLogStats(query?)` (real backend uses `GET /api/stats`)
