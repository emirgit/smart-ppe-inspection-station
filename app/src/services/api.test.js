...
describe('API — Roles', () => {
  ...
  it('updates a role', async () => {
    const role = await api.createRole({ role_name: 'Role To Update', description: 'old' });
    const res = await api.updateRole(role.data.id, { role_name: 'Updated Role', description: 'new' });
    expect(res.data.role_name).toBe('Updated Role');
    expect(res.data.description).toBe('new');
  });
...
describe('API — PPE Items', () => {
  ...
  it('updates a PPE item', async () => {
    const item = await api.createPpeItem({ item_key: 'test_ppe_item', display_name: 'Test PPE' });
    const res = await api.updatePpeItem(item.data.id, { display_name: 'Updated PPE' });
    expect(res.data.display_name).toBe('Updated PPE');
  });
});
