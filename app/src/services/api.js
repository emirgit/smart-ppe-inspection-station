...
const mockApi = {
  ...
  async updateRole(id, body) {
    ...
    if (body.role_name && mockRoles.some(r => r.id !== id && r.role_name.toLowerCase() === body.role_name.toLowerCase())) {
      throw new ApiError(409, 'Role name already exists');
    }
  ...
  async updatePpeItem(id, body) {
    ...
    if (body.item_key && mockPpeItems.some(p => p.id !== id && p.item_key === body.item_key)) {
      throw new ApiError(409, 'PPE item key already exists');
    }
    mockPpeItems[idx] = { ...mockPpeItems[idx], ...body };
    return { success: true, data: mockPpeItems[idx] };
  },
