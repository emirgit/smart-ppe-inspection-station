...
export function Analytics() {
  ...
  async function load(days: number) {
    ...
    try { const res = await api.getEntryLogStats(getRange(days)); setStats(res.data) }
      ...
      <Main>
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            {RANGES.map(r => (
              <Button key={r.days} size="sm" variant={rangeDays === r.days ? 'default' : 'outline'} onClick={() => setRangeDays(r.days)}>{r.label}</Button>
            ))}
          </div>
        </div>
        ...
        {error && (
          <div className="flex items-center gap-2 p-4 mb-4 rounded-lg border border-destructive/50 bg-destructive/10 text-destructive text-sm">
            ...
            <Button size="sm" variant="ghost" className="ms-auto" onClick={() => load(rangeDays)}>Tekrar dene</Button>
          </div>
        )}
        ...
        <div className="grid gap-4 grid-cols-2 md:grid-cols-4 mb-6">
          {[
            ...
            <Card key={label}>
              ...
              <CardContent>{loading ? <Skeleton className="h-7 w-16" /> : <p className="text-xl font-bold">{value ?? '—'}</p>}</CardContent>
            </Card>
          ))}
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 mb-6">
          <Card>
            ...
            <CardContent>
              {loading ? <Skeleton className="h-48 w-full" /> : !stats?.daily_data?.length
                ...
                : <ResponsiveContainer width="100%" height={200}>
                    <LineChart data={stats.daily_data}>
                      ...
                      <Tooltip formatter={(v: any) => [`%${v}`, 'Uyum']} labelFormatter={(l: string) => `Tarih: ${l}`} />
              ...
              {loading ? <Skeleton className="h-48 w-full" /> : !stats?.daily_data?.length
                ...
                : <ResponsiveContainer width="100%" height={200}>
                    <BarChart data={stats.daily_data}>
                      ...
                      <Tooltip labelFormatter={(l: string) => `Tarih: ${l}`} />
                      <Legend formatter={(v: string) => v === 'pass' ? 'Geçti' : 'Başarısız'} />
                      <Bar dataKey="pass" fill="#22c55e" radius={[2, 2, 0, 0]} />
        ...
        <Card>
          ...
          <CardContent>
            {loading ? <div className="space-y-3">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-8 w-full" />)}</div>
